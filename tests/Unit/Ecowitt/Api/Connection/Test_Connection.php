...
class Test_Connection extends \WP_UnitTestCase {
	...
	public function set_up(): void {
		...
		$this->sample_data = array(
			'key'             => 'test_key_123',
			'api_key'         => 'api_key_456',
			'application_key' => 'application_key_789',
	...
	public function test_can_create_connection_instance(): void {
		$connection = new Connection(
			...
			$this->sample_data['application_key'],
	 ...
	 * @covers \PinkCrab\Ecowitt_Weather_Block\Ecowitt\Api\Connection\Connection::application_key
	 */
	public function test_can_get_application_key(): void {
		$connection = new Connection(
			$this->sample_data['key'],
			$this->sample_data['application_key'],
			$this->sample_data['api_key'],
		...
		$this->assertSame( $this->sample_data['application_key'], $connection->application_key() );
	}
	...
	public function test_constructor_escapes_html_attributes(): void {
		...
		$connection = new Connection(
			$malicious_data['key'],
			$malicious_data['application_key'],
		...
		$this->assertSame( esc_attr( $malicious_data['application_key'] ), $connection->application_key() );
	...
	public function test_implements_json_serializable(): void {
		...
		$expected_keys = array( 'key', 'api_key', 'application_key', 'mac_address', 'description', 'name' );
		...
		$this->assertSame( $this->sample_data['application_key'], $serialized['application_key'] );
	...
	public function test_handles_empty_string_values(): void {
		...
		$this->assertSame( '', $connection->application_key() );
	...
	public function test_handles_special_characters(): void {
		$special_data = array(
			...
			'application_key' => 'secret_with_$pecial_ch@rs!',
		...
		$this->assertSame( esc_attr( $special_data['application_key'] ), $connection->application_key() );
