...
class Temp_Channels extends Component {
	 ...
	 *
	 * @var array<array{temperature: Temperature|null, channel_name: string}>
	 */
	 ...
	 *
	 * @param array<string, array<string, mixed>> $measurements Array of temperature channel measurement arrays
	 */
	public function __construct( array $measurements = array() ) {
		foreach ( $measurements as $channel_key => $channel_data ) {
			...
			if ( isset( $channel_data['temperature'] ) ) {
				$channel['temperature'] = new Temperature( $channel_data['temperature'], _x( 'Temperature', 'temp channels measurement label', 'ecowitt-weather-block' ) );
			}
